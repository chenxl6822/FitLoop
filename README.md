...
当前基线：后端 154 项单元/切片测试和 2 项 MySQL/Testcontainers 集成测试通过，JaCoCo 覆盖率门禁通过；Agent 18 项测试通过；Flutter 46 项测试和静态分析通过。教练与申诉审批已使用真实 DeepSeek V4 模型完成模型层演示，且两条完整应用链路已通过隔离容器 E2E。
...
- `0.1.6+7` 作为作品集候选版本已进入证据固化与演示体验完善阶段；普通用户 AI 教练 UI 已通过 PR #5 合入 `main`，本周期尚未部署或发布 APK。
- Agent 真实模型演示使用固定脱敏证据验证模型、工具调用、结构化输出和护栏；隔离容器 E2E 验证 Spring、Redis、Worker、工具审计和人工确认的完整链路。
- 当前已验证证据仅覆盖运行 `0.1.5+6` 的公网旧版；是否保留旧版测试、升级为安全演示或停止公网服务尚待 P0 决策。该环境在版本同步且 HTTPS 验证完成前不代表当前 `main`，也不作为生产环境。
- TLS、证书到期监控和 Agent 降级配置保留为工程能力展示；若决定保留或升级公网服务，需完成域名、证书与 HTTPS 验证；若决定下线，则按决策停止服务。
