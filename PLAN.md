...
- [ ] **任务2**：运动打卡结束后展示真实结果（替代写死的 _lastRecord）
  - 状态：待执行
  - 验收标准：
...
- [ ] **任务3**：接入本地缓存（SharedPreferences 或 Hive）
  - 状态：待执行
  - 验收标准：
    - [ ] 用户token持久化（重启App自动登录）
    - [ ] 目标和统计数据缓存（离线可查看上次数据）
    - [ ] 网络恢复后自动同步打卡记录

### P2 - 通知与提醒

- [ ] **任务4**：接入本地通知（flutter_local_notifications）
  - 状态：待执行
  - 验收标准：
    - [ ] 提醒配置页（我的页面→提醒设置）
    - [ ] 运动提醒、久坐提醒、喝水提醒、睡眠提醒可配置
    - [ ] 通知权限引导（首次触发时请求权限）

### P3 - 图表与可视化

- [ ] **任务5**：统计页面接入图表（fl_chart 或 syncfusion_flutter_charts）
  - 状态：待执行
  - 验收标准：
    - [ ] 本周/本月运动次数柱状图
    - [ ] 体重趋势折线图（来自健康数据）
